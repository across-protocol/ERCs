...
A cross-chain compliant order type MUST be ABI decodable into either `SponsoredOrder` or `OnchainOrder` type. The `SponsoredOrder` is designed to be signed by users who want to delegate submission of the order creation transaction to another account (i.e. a "sponsor"). These structs were designed originally to capture cross-chain user intents but they could theoretically be used for creating single-chain intents as well.

```solidity
/// @title SponsoredOrder type
...
interface IOriginSettler {
	...
	/// @param order The SponsoredOrder definition
	/// @param signature The user's signature over the order
	/// @param originFillerData Any filler-defined data required by the settler
	function openFor(SponsoredOrder calldata order, bytes calldata signature, bytes calldata originFillerData) external;

	/// @notice Opens a cross-chain order
	/// @dev To be called by the user
	/// @dev This method must emit the Open event
	/// @param order The OnchainOrder definition
	function open(OnchainOrder calldata order) external;

	/// @notice Resolves a specific SponsoredOrder into a generic ResolvedCrossChainOrder
	/// @dev Intended to improve standardized integration of various order types and settlement contracts
	/// @param order The SponsoredOrder definition
	/// @param originFillerData Any filler-defined data required by the settler
	/// @return ResolvedCrossChainOrder hydrated order data including the inputs and outputs of the order
	function resolveFor(SponsoredOrder calldata order, bytes calldata originFillerData) external view returns (ResolvedCrossChainOrder memory);

	/// @notice Resolves a specific OnchainOrder into a generic ResolvedCrossChainOrder
	/// @dev Intended to improve standardized integration of various order types and settlement contracts
	/// @param order The OnchainOrder definition
	/// @return ResolvedCrossChainOrder hydrated order data including the inputs and outputs of the order
	function resolve(OnchainOrder calldata order) external view returns (ResolvedCrossChainOrder memory);
}
```
